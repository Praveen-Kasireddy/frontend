 ...
 */
mxCellEditor.prototype.emptyLabelText = mxClient.IS_FF ? '<br>' : '<p></p>';
...
mxCellEditor.prototype.getInitialValue = function(state, trigger) {
    ...
    if (
        ...
        result = mxUtils.replaceTrailingNewlines(result, '<p><br></p>');
    }
        ...
        if (!this.autoSize || state.style[mxConstants.STYLE_OVERFLOW] == 'fill') {
            ...
            if (isEdge) {
                ...
                if (
                    ...
                    var spacingTop =
                        (parseInt(state.style[mxConstants.STYLE_SPACING_TOP] || 0) + mxText.prototype.baseSpacingTop) *
                            scale +
                        spacing;
                    var spacingRight =
            ...
            if (
                ...
                var tmp =
                    Math.round(this.bounds.width / (document.documentMode == 8 ? scale : scale)) + this.wordWrapPadding;
            ...
            if (document.documentMode == 8) {
                ...
                this.textarea.style.left =
                    Math.max(0, Math.round(this.bounds.x - m.x * (this.bounds.width - 2)) + 1) + 'px';
                this.textarea.style.top =
                    Math.max(0, Math.round(this.bounds.y - m.y * (this.bounds.height - 4) + (m.y == -1 ? 3 : 0)) + 1) +
                    'px';
            }
        }
...
mxCellEditor.prototype.startEditing = function(cell, trigger) {
    ...
    if (state != null) {
        ...
        if (
            this.autoSize &&
            (this.graph.model.isEdge(state.cell) || state.style[mxConstants.STYLE_OVERFLOW] != 'fill')
        ) {
            window.setTimeout(
                mxUtils.bind(this, function() {
                    this.resize();
...
mxCellEditor.prototype.stopEditing = function(cancel) {
    ...
    if (this.editingCell != null) {
        ...
        this.textarea = null;
        // edited by KPMG - CT for force refreshing after editing
        this.graph.refresh();
    }
};
...
mxCellEditor.prototype.prepareTextarea = function() {
    if (mxClient.IS_FF && this.textarea.lastChild != null && this.textarea.lastChild.nodeName == 'BR') {
