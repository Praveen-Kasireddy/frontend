...
(function() {
     ...
     *
     */
    Editor.spinImage = !mxClient.IS_SVG
        ? IMAGE_PATH + '/spin.gif'
     ...
     * Blank 1x1 pixel transparent PNG image.
     */
    Editor.hiResImage = mxClient.IS_SVG
     ...
     * Blank 1x1 pixel transparent PNG image.
     */
    Editor.loResImage = mxClient.IS_SVG
    ...
    Editor.configure = function(config) {
        if (config != null) {
            ...
            StyleFormatPanel.prototype.defaultColorSchemes =
                config.defaultColorSchemes || StyleFormatPanel.prototype.defaultColorSchemes;
    ...
    if (window.StyleFormatPanel != null) {
         ...
         * for style color picker
         * changed to kpmg colors
         * 24.05.2018 KDEV_FMEXT
         */
        StyleFormatPanel.prototype.defaultColorSchemes = [
            [
                null,
                { fill: '#00338D', stroke: '#00338D' },
                { fill: '#005EB8', stroke: '#005EB8' },
                { fill: '#0091DA', stroke: '#0091DA' },
                { fill: '#483698', stroke: '#483698' },
                { fill: '#470A68', stroke: '#470A68' },
                { fill: '#6D2077', stroke: '#6D2077' },
                { fill: '#00A3A1', stroke: '#00A3A1' }
            ],
            [
                { fill: '#000000', stroke: '#000000' },
                { fill: '#009A44', stroke: '#009A44' },
                { fill: '#43B02A', stroke: '#43B02A' },
                { fill: '#EAAA00', stroke: '#EAAA00' },
                { fill: '#F68D2E', stroke: '#F68D2E' },
                { fill: '#BC204B', stroke: '#BC204B' },
                { fill: '#C6007E', stroke: '#C6007E' },
            ...
            [
                ...
                { fill: '#43B02A', stroke: '#43B02A' },
                { fill: '#C6007E', stroke: '#C6007E' },
                { fill: '#753F19', stroke: '#753F19' },
                { fill: '#9B642E', stroke: '#9B642E' },
                { fill: '#909375', stroke: '#909375' },
                { fill: '#E38C9F', stroke: '#E38C9F' },
                { fill: '#E36877', stroke: '#E36877' }
            ]
        ];
        ...
        StyleFormatPanel.prototype.addStyleOps = function(div) {
            ...
            btn.setAttribute(
                'title',
                mxResources.get('pasteStyle') + ' (' + this.editorUi.actions.get('pasteStyle').shortcut + ')'
            );
            btn.style.marginBottom = '2px';
            btn.style.width = '100px';
        ...
        StyleFormatPanel.prototype.addStyles = function(div) {
                        ...
                        if (colorset['gradient'] != null) {
                            if (mxClient.IS_IE && (mxClient.IS_QUIRKS || document.documentMode < 10)) {
                                ...
                                btn.style.backgroundImage =
                                    'linear-gradient(' + colorset['fill'] + ' 0px,' + colorset['gradient'] + ' 100%)';
                            }
                        } else {
            ...
            left.style.cssText =
                'position:absolute;left:10px;top:8px;bottom:8px;width:20px;margin:4px;opacity:0.5;' +
            ...
            mxEvent.addListener(
                ...
                mxUtils.bind(this, function() {
                    this.editorUi.currentScheme = mxUtils.mod(
                        this.editorUi.currentScheme - 1,
            ...
            right.style.cssText =
                'position:absolute;left:202px;top:8px;bottom:8px;width:20px;margin:4px;opacity:0.5;' +
    ...
    Graph.prototype.addSvgShadow = function(svgRoot, group, createOnly) {
        ...
        var filter =
            svgDoc.createElementNS != null
                ? svgDoc.createElementNS(mxConstants.NS_SVG, 'filter')
                : svgDoc.createElement('filter');
        filter.setAttribute('id', this.shadowId);

        var blur =
            svgDoc.createElementNS != null
                ? svgDoc.createElementNS(mxConstants.NS_SVG, 'feGaussianBlur')
                : svgDoc.createElement('feGaussianBlur');
        ...
        var offset =
            svgDoc.createElementNS != null
                ? svgDoc.createElementNS(mxConstants.NS_SVG, 'feOffset')
                : svgDoc.createElement('feOffset');
        ...
        var flood =
            svgDoc.createElementNS != null
                ? svgDoc.createElementNS(mxConstants.NS_SVG, 'feFlood')
                : svgDoc.createElement('feFlood');
        ...
        var composite =
            svgDoc.createElementNS != null
                ? svgDoc.createElementNS(mxConstants.NS_SVG, 'feComposite')
                : svgDoc.createElement('feComposite');
        ...
        var feBlend =
            svgDoc.createElementNS != null
                ? svgDoc.createElementNS(mxConstants.NS_SVG, 'feBlend')
                : svgDoc.createElement('feBlend');
    ...
    PrintDialog.prototype.create = function(editorUi, titleText) {
        ...
        function validatePageRange() {
            pagesToInput.value = Math.max(
                1,
                Math.min(pageCount, Math.max(parseInt(pagesToInput.value), parseInt(pagesFromInput.value)))
            );
            pagesFromInput.value = Math.max(
                1,
                Math.min(pageCount, Math.min(parseInt(pagesToInput.value), parseInt(pagesFromInput.value)))
            );
        }
