...
(function() {
    ...
    Menus.prototype.init = function() {
            ...
            new Action(mxResources.get('url') + '...', function() {
                editorUi.showPublishLinkDialog(mxResources.get('url'), true, null, null, function(
                    ...
                    var dlg = new EmbedDialog(
                        editorUi,
                        editorUi.createLink(linkTarget, linkColor, allPages, lightbox, editLink, layers, null, true)
                    );
            ...
            new Action(mxResources.get('formatPdf') + '...', function() {
                if (editorUi.isOffline() || editorUi.printPdfExport) {
                    ...
                    editorUi.showDialog(
                        ...
                        360,
                        editorUi.pages != null && editorUi.pages.length > 1 ? 420 : 360,
                        true,
                        true
                    );
                } else {
        ...
        editorUi.actions.addAction('revisionHistory...', function() {
            ...
            if (
                file == null ||
                (file.constructor != DriveFile && file.constructor != DropboxFile) ||
                ...
                if (file.constructor == DropboxFile) {
                    ...
                    var promise = editorUi.dropbox.client.filesListRevisions({
                        path: file.stat.path_lower,
                        limit: 100
                    });
                    promise.then(
                        mxUtils.bind(this, function(response) {
                            ...
                            try {
                                ...
                                for (var i = response.entries.length - 1; i >= 0; i--) {
                                    (function(stat) {
                                        revs.push({
                                            modifiedDate: stat.client_modified,
                                            fileSize: stat.size,
                                            getXml: function(success, error) {
                                                editorUi.dropbox.readFile(
                                                    { path: file.stat.path_lower, rev: stat.rev },
                                                    success,
                                                    error
                                                );
                                            },
                ...
                // Google Drive File
                    ...
                    editorUi.drive.executeRequest(
                        gapi.client.drive.revisions.list({ fileId: file.getId() }),
                        function(resp) {
                            editorUi.spinner.stop();
                            ...
                            for (var i = 0; i < resp.items.length; i++) {
                                (function(item) {
                                    item.getXml = function(success, error) {
                                        ...
                                        editorUi.drive.executeRequest(
                                            gapi.client.drive.revisions.get({
                                                fileId: file.getId(),
                                                revisionId:
                                                    resp.items[resp.items.length - 1] === item
                                                        ? file.desc.headRevisionId
                                                        : item.id
                                            }),
                                            function(resp) {
                                                editorUi.drive.getXmlFile(
                                                    resp,
                                                    null,
                                                    function(file2) {
                                    ...
                                    item.getUrl = function() {
                                        return (
                                            editorUi.getUrl(
                                                window.location.pathname + '?rev=' + item.id + '&chrome=0&edit=_blank'
                                            ) + window.location.hash
                                        );
                                    };
                                })(resp.items[i]);
                ...
                mxUtils.bind(this, function(menu, parent) {
                            ...
                            mxUtils.bind(this, function(e) {
                                ...
                                if (e.keyCode == 13 && term.length > 0) {
                                    this.editorUi.openLink(
                                        'https://desk.draw.io/support/search/solutions?term=' + encodeURIComponent(term)
                                    );
                    ...
                    if (urlParams['test'] == '1') {
                                ...
                                if (!graph.isSelectionEmpty()) {
                                    ...
                                    mxLog.debug(
                                        "sb.createVertexTemplateFromData('" +
                                            graph.compress(
                                                mxUtils.getXml(graph.encodeCells(graph.getSelectionCells()))
                                            ) +
                                            "', width, height, 'Title');"
                                    );
                                }
                        ...
                        if (this.editorUi.getCurrentFile() != null && this.editorUi.getCurrentFile().realtime != null) {
                            this.addMenuItems(
                                menu,
                                ['-', 'testShowRtModel', 'testDebugRtModel', 'testDownloadRtModel'],
                                parent
                            );
                        }
                                    ...
                                    mxUtils.bind(this, function(html, scriptTag) {
                                        var dlg = new EmbedDialog(
                                            ...
                                            function() {
                                                ...
                                                var direct =
                                                    mxClient.IS_IE || mxClient.IS_EDGE || document.documentMode != null;
                ...
                editorUi.showPublishLinkDialog(null, null, null, null, function(
                    ...
                    if (editorUi.spinner.spin(document.body, mxResources.get('loading'))) {
                        editorUi.getPublicUrl(editorUi.getCurrentFile(), function(url) {
                            ...
                            var dlg = new EmbedDialog(
                                editorUi,
                                editorUi.createLink(linkTarget, linkColor, allPages, lightbox, editLink, layers, url)
                            );
                            editorUi.showDialog(dlg.container, 440, 240, true, true);
                            dlg.init();
                        });
        ...
        if (urlParams['embed'] == '1') {
            editorUi.actions.get('save').funct = function(exit) {
                ...
                var data =
                    urlParams['pages'] != '0' || (editorUi.pages != null && editorUi.pages.length > 1)
        ...
        this.put(
            ...
            new Menu(
                mxUtils.bind(this, function(menu, parent) {
                    ...
                    if (!editorUi.isOffline()) {
                        menu.addSeparator(parent);
                        this.addMenuItem(menu, 'export', parent).firstChild.nextSibling.innerHTML =
        ...
        this.put(
            ...
            new Menu(function(menu, parent) {
                ...
                var doImportFile = mxUtils.bind(this, function(data, mime, filename) {
                    ...
                    var x = graph.snap(
                        Math.ceil(Math.max(0, bds.x / view.scale - view.translate.x) + 4 * graph.gridSize)
                    );
                    var y = graph.snap(
                        Math.ceil(
                            Math.max(0, (bds.y + bds.height) / view.scale - view.translate.y) + 4 * graph.gridSize
                        )
                    );
                                    ...
                                    editorUi.resizeImage(
                                        ...
                                        mxUtils.bind(this, function(data2, w2, h2) {
                                            var s = resizeImages
                                                ? Math.min(
                                                      1,
                                                      Math.min(editorUi.maxImageSize / w2, editorUi.maxImageSize / h2)
                                                  )
                                                : 1;
                                ...
                                file.rename(
                                    ...
                                    mxUtils.bind(this, function(resp) {
                                        this.editorUi.handleError(
                                            resp,
                                            resp != null ? mxResources.get('errorRenamingFile') : null
                                        );
                                    })
                                );
                            }
        ...
        editorUi.actions.put(
            'offline',
            new Action(mxResources.get('offline') + '...', function() {
        ...
        editorUi.actions.put(
            'download',
            new Action(mxResources.get('download') + '...', function() {
        ...
        // 29.05.2018 - KDEV_CT -> remove because it dont work with our version
        // var methods = [
        //     'horizontalFlow',
        //     'verticalFlow',
        //     '-',
        ...
        this.put(
            ...
            new Menu(
                mxUtils.bind(this, function(menu, parent) {
                    this.addMenuItems(
                        menu,
                        (this.editorUi.format != null ? ['formatPanel'] : []).concat(['outline', 'layers', '-'])
                    );
                    ...
                    this.addMenuItems(
                        menu,
                        ['shapes', '-', 'pageView', 'pageScale', '-', 'scrollbars', 'tooltips', '-', 'grid', 'guides'],
                        parent
                    );
                    ...
                    this.addMenuItems(
                        menu,
                        ['-', 'connectionArrows', 'connectionPoints', '-', 'resetView', 'zoomOut'],
                        parent
                    );
                })
            )
