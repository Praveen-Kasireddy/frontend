...
Editor.prototype.editAsNew = function(xml, title) {
    ...
    if (this.editorWindow != null && !this.editorWindow.closed) {
        ...
        if (
            ...
            this.editorWindow = this.graph.openLink(this.getEditBlankUrl(p + (p.length > 0 ? '&' : '?') + 'client=1'));
        } else {
            this.editorWindow = this.graph.openLink(this.getEditBlankUrl(p) + '#R' + encodeURIComponent(xml));
        }
    }
};
