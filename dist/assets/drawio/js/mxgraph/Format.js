...
//03.05.2018 KDEV_CT
Format.prototype.inactiveTabBackgroundColor = '#707070';
...
ArrangePanel.prototype.addEdgeGeometry = function(container) {
    ...
    function widthUpdate(evt) {
        ...
        if (
            value !=
            mxUtils.getValue(rect.style, 'width', mxCellRenderer.defaultShapes['flexArrow'].prototype.defaultWidth)
        ) {
...
TextFormatPanel.prototype.addFont = function(container) {
    ...
    var left = this.editorUi.toolbar.addButton(
        ...
        graph.cellEditor.isContentEditing()
            ...
            : callFn(
                  this.editorUi.menus.createStyleChangeFunction([mxConstants.STYLE_ALIGN], [mxConstants.ALIGN_LEFT])
              ),
        stylePanel3
    );
    var center = this.editorUi.toolbar.addButton(
        ...
        graph.cellEditor.isContentEditing()
            ...
            : callFn(
                  this.editorUi.menus.createStyleChangeFunction([mxConstants.STYLE_ALIGN], [mxConstants.ALIGN_CENTER])
              ),
        stylePanel3
    );
    var right = this.editorUi.toolbar.addButton(
        ...
        graph.cellEditor.isContentEditing()
            ...
            : callFn(
                  this.editorUi.menus.createStyleChangeFunction([mxConstants.STYLE_ALIGN], [mxConstants.ALIGN_RIGHT])
              ),
        stylePanel3
    );
    ...
    var top = this.editorUi.toolbar.addButton(
        ...
        mxResources.get('top'),
        callFn(
            this.editorUi.menus.createStyleChangeFunction([mxConstants.STYLE_VERTICAL_ALIGN], [mxConstants.ALIGN_TOP])
        ),
        stylePanel3
    );
    var middle = this.editorUi.toolbar.addButton(
                    ...
                    if (node != null) {
                        ...
                        if (css != null) {
                            setSelected(
                                fontStyleItems[0],
                                css.fontWeight == 'bold' ||
                                    graph.getParentByName(node, 'B', graph.cellEditor.textarea) != null
                            );
                            setSelected(
                                fontStyleItems[1],
                                css.fontStyle == 'italic' ||
                                    graph.getParentByName(node, 'I', graph.cellEditor.textarea) != null
                            );
                            setSelected(
                                fontStyleItems[2],
                                graph.getParentByName(node, 'U', graph.cellEditor.textarea) != null
                            );
                            ...
                            // Converts rgb(r,g,b) values
                            var color = css.color.replace(/\brgb\s*\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)/g, function(
                                $0,
                                $1,
                                $2,
                                $3
...
DiagramFormatPanel.prototype.init = function() {
    ...
    if (graph.isEnabled()) {
        ...
        // 2018-07-02 - KDEV_AH - disable paper size settings
