...
mxText.prototype.paint = function(c, update) {
    ...
    if (update) {
        ...
        if (fmt == 'html' && !mxUtils.isNode(this.value)) {
            val = mxUtils.replaceTrailingNewlines(val, '<p><br></p>');
        }
...
mxText.prototype.apply = function(state) {
    ...
    if (this.style != null) {
        ...
        this.spacingRight =
            parseInt(mxUtils.getValue(this.style, mxConstants.STYLE_SPACING_RIGHT, this.spacingRight - old)) +
            this.spacing;
        this.spacingBottom =
            parseInt(mxUtils.getValue(this.style, mxConstants.STYLE_SPACING_BOTTOM, this.spacingBottom - old)) +
            this.spacing;
        this.spacingLeft =
            parseInt(mxUtils.getValue(this.style, mxConstants.STYLE_SPACING_LEFT, this.spacingLeft - old)) +
            this.spacing;
...
mxText.prototype.updateHtmlTransform = function() {
    ...
    style.left =
        Math.round(this.bounds.x - Math.ceil(dx * (this.overflow != 'fill' && this.overflow != 'width' ? 3 : 1))) +
        'px';
...
mxText.prototype.updateInnerHtml = function(elt) {
    if (mxUtils.isNode(this.value)) {
        ...
        // Handles trailing newlines to make sure they are visible in rendering output
        val = mxUtils.replaceTrailingNewlines(val, '<p>&nbsp;</p>');
