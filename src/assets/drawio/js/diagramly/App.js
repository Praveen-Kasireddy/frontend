...
App.main = function(callback, createUi) {
        ...
        if (urlParams['plugins'] != '0' && urlParams['offline'] != '1') {
            ...
            if (plugins != null && plugins.length > 0 && urlParams['plugins'] != '0') {
                ...
                if (
                    local ||
                    mxUtils.confirm(
                        mxResources.replacePlaceholders(warning, [plugins.join('\n')]).replace(/\\n/g, '\n')
                    )
                ) {
    ...
    mxUtils.getAll(
        urlParams['dev'] != '1'
            ? [bundle]
            : [bundle, uiTheme == 'dark' ? STYLE_PATH + '/dark-default.xml' : STYLE_PATH + '/default.xml'],
        function(xhr) {
            ...
            // Main
            // KDEV_CT 24.08.18 - fix to get draw.io into kosmos layout
            var ui =
                createUi != null
                    ? createUi()
                    : new App(new Editor(urlParams['chrome'] == '0'), document.getElementById('drawIOContainer'));
...
App.prototype.init = function() {
    ...
    this.gitHub =
        ...
        (urlParams['gh'] != '0' && (urlParams['embed'] != '1' || urlParams['gh'] == '1'))
        ...
        var initDriveClient = mxUtils.bind(this, function() {
            ...
            if (typeof gapi !== 'undefined') {
                ...
                if (window.DrawGapiClientCallback != null) {
                    gapi.load(
                        (urlParams['picker'] != '0' ? 'picker,' : '') + 'auth:client,drive-realtime,drive-share',
...
App.prototype.checkLicense = function() {
    ...
    var email = (urlParams['dev'] == '1' ? urlParams['lic'] : null) || (driveUser != null ? driveUser.email : null);
                ...
                if (diff < 90) {
                    ...
                    if (diff < 0) {
                        ...
                        footer.innerHTML =
                            ...
                            mxResources.get('licenseHasExpired', [domain, exp.toLocaleDateString()]) +
                            '</a></td></tr></table>';
                    } else {
                        ...
                        footer.innerHTML =
                            ...
                            mxGraph.prototype.warningImage.src +
                            '" align="top" style="margin-right:6px">' +
                            mxResources.get('licenseWillExpire', [domain, exp.toLocaleDateString()]) +
                            '</a></td></tr></table>';
                    }
...
App.prototype.updateActionStates = function() {
    ...
    this.actions
        .get('revisionHistory')
        .setEnabled(
            file != null && ((file.constructor == DriveFile && file.isEditable()) || file.constructor == DropboxFile)
        );
};
...
App.prototype.showSplash = function(force) {
    ...
    var showSecondDialog = mxUtils.bind(this, function() {
        ...
        this.showDialog(
            dlg.container,
            340,
            serviceCount < 2 || mxClient.IS_CHROMEAPP || EditorUi.isElectronApp ? 200 : 260,
            true,
            true,
            mxUtils.bind(this, function(cancel) {
    ...
    if (this.editor.chromeless) {
        ...
        this.showDialog(dlg.container, rowLimit < 3 ? 260 : 300, serviceCount >= 4 ? 420 : 300, true, false);
        dlg.init();
...
EditorUi.prototype.loadTemplate = function(url, onload, onerror) {
    ...
    this.loadUrl(
        ...
        onerror,
        /(\.png)($|\?)/i.test(url) || /(\.vsdx)($|\?)/i.test(url)
    );
};
...
App.prototype.fileCreated = function(file, libs, replace, done) {
    ...
    if (this.spinner.spin(document.body, mxResources.get('inserting'))) {
        ...
        var dataNode =
            data.length > 0 ? this.editor.extractGraphModel(mxUtils.parseXml(data).documentElement, true) : null;
                        ...
                        if (text != null && text.length > 0) {
                            ...
                            if (urlParams['title'] == null && urlParams['notitle'] != '1') {
                                ...
                                if (dot > slash && slash > 0) {
                                    ...
                                    if (ext === '.svg' || ext === '.xml' || ext === '.html' || ext === '.png') {
                                        filename = tmp + ext;
                                    }
                                }
                            }
...
App.prototype.convertFile = function(url, filename, mimeType, extension, success, error) {
    ...
    if (
        (/\.vsdx$/i.test(filename) || /\.vsd$/i.test(filename)) &&
        Graph.fileSupport &&
                        ...
                        var addUser = mxUtils.bind(this, function(user, logo, logout) {
                            if (user != null) {
                                ...
                                this.userPanel.innerHTML +=
                                    ...
                                    '</b>' +
                                    (user.email != null
                                        ? '<br><font color="gray">' + mxUtils.htmlEntities(user.email) + '</font></td>'
                                        : '') +
                                    '</tr></table>';
                                var div = document.createElement('div');
                                div.style.textAlign = 'center';
