...
// Paths and files
window.SHAPES_PATH = window.SHAPES_PATH || document.head.getElementsByTagName('base')[0].href + '/assets/drawio/shapes';
// Path for images inside the diagram
window.GRAPH_IMAGE_PATH =
    window.GRAPH_IMAGE_PATH || document.head.getElementsByTagName('base')[0].href + '/assets/drawio/img';
window.ICONSEARCH_PATH =
    window.ICONSEARCH_PATH ||
    ((navigator.userAgent.indexOf('MSIE') >= 0 || urlParams['dev']) && window.location.protocol != 'file:'
        ? '/assets/drawio/iconSearch'
        : 'https://www.draw.io/iconSearch');
window.TEMPLATE_PATH =
    window.TEMPLATE_PATH || document.head.getElementsByTagName('base')[0].href + '/assets/drawio/templates';
...
(function() {
    ...
    if (typeof JSON !== 'undefined') {
        ...
        if (isLocalStorage) {
            try {
                // KDEV_CT 24.08.18 - don't show start / splash screen
                urlParams['splash'] = '0';
                /*   var value = localStorage.getItem('.drawio-config');
                var showSplash = true;
