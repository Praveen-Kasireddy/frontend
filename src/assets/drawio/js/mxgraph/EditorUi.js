...
EditorUi.prototype.getCssClassForMarker = function(prefix, shape, marker, fill) {
    ...
    if (shape == 'flexArrow') {
        ...
        if (marker == mxConstants.ARROW_CLASSIC) {
            ...
            result =
                fill == '1' ? 'geSprite geSprite-' + prefix + 'block' : 'geSprite geSprite-' + prefix + 'blocktrans';
            ...
            result =
                fill == '1' ? 'geSprite geSprite-' + prefix + 'async' : 'geSprite geSprite-' + prefix + 'asynctrans';
    ...
    if (this.editor.chromeless) {
        resize = mxUtils.bind(this, function(autoscale, maxScale, cx, cy) {
            if (graph.container != null) {
                ...
                var dy =
                    this.lightboxVerticalDivider == 0 ? 0 : (ch - ns * b.height) / this.lightboxVerticalDivider / ns;
        ...
        if (urlParams['toolbar'] != '0') {
            ...
            mxEvent.addListener(this.chromelessToolbar, mxClient.IS_POINTER ? 'pointermove' : 'mousemove', function(
                evt
            ) {
                mxEvent.consume(evt);
            });
...
EditorUi.prototype.updateActionStates = function() {
    ...
    this.actions
        .get('group')
        .setEnabled(
            graph.getSelectionCount() > 1 || (oneVertexSelected && !graph.isContainer(graph.getSelectionCell()))
        );
    this.actions
        .get('ungroup')
        .setEnabled(
    ...
    this.actions
        .get('removeFromGroup')
        .setEnabled(
            oneVertexSelected && graph.getModel().isVertex(graph.getModel().getParent(graph.getSelectionCell()))
        );
...
EditorUi.prototype.refresh = function(sizeDidChange) {
    ...
    this.sidebarContainer.setAttribute('style', 'position: absolute !important;');
    this.sidebarContainer.setAttribute('style', this.sidebarContainer.getAttribute('style') + '; overflow: auto !important');
    ...
    this.diagramContainer.style.left =
        this.hsplit.parentNode != null ? effHsplitPosition + this.splitSize + 'px' : '0px';
...
// */
    ...
    if (this.format != null) {
        // 02.05.2018 - KDEV_CT
        var element = document.getElementById('reportsidebar');
        element.appendChild(this.formatContainer);
...
EditorUi.prototype.pickColor = function(color, apply) {
    ...
    // Size changed
    // 23.05.2018 KDEV_FMEXT
    this.showDialog(dlg.container, 211, 450, true, false); //291, 530
    dlg.init();
};
...
EditorUi.prototype.createKeyHandler = function(editor) {
    ...
    if (!this.editor.chromeless || this.editor.editable) {
        ...
        keyHandler.bindAction(65, true, 'advancedEdit', true); // Ctrl+Shift+A
