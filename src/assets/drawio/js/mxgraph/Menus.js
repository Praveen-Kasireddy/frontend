 ...
 */
Menus.prototype.defaultFont = 'UniversLTStd-55Roman';
    ...
    this.put(
        ...
        new Menu(
            mxUtils.bind(this, function(menu, parent) {
                ...
                addItem('', 'p').firstChild.nextSibling.innerHTML = '<p>' + mxResources.get('paragraph') + '</p>';
                addItem('', 'pre').firstChild.nextSibling.innerHTML = '<pre>' + mxResources.get('quote') + '</pre>';
            })
        )
    );
    this.put(
...
Menus.prototype.pickColor = function(key, cmd, defaultValue) {
    ...
    if (cmd != null && graph.cellEditor.isContentEditing()) {
        ...
        // changed size uper color picker
        // 24.05.2018 KDEV_FMEXT
        this.editorUi.showDialog(dlg.container, 211, 450, true, true); // 291, 530
        dlg.init();
    } else {
        ...
        // changed size uper color picker
        // 24.05.2018 KDEV_FMEXT
        this.editorUi.showDialog(this.colorDialog.container, 211, 450, true, true); // 291, 530
...
Menus.prototype.createPopupMenu = function(menu, cell, evt) {
    ...
    if (!graph.isSelectionEmpty()) {
        ...
        if (state != null) {
            ...
            if (graph.getSelectionCount() == 1) {
                menu.addSeparator();
                this.addMenuItems(menu, ['edit', '-', 'editData', 'editLink', 'advancedEdit'], null, evt);
        ...
        this.addMenuItems(
            menu,
            ['-', 'selectVertices', 'selectEdges', 'selectAll', '-', 'clearDefaultStyle'],
            null,
            evt
        );
    }
